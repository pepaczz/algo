...
for year in years:
    ...
    for q in year_folders:
        ...
        sub_list.append(pd.concat([chunk[chunk['form'] == '10-K'] for chunk in sub_read_iter]))

    # filter sp500 companies only
    sub = pd.concat(sub_list)
    sp500_adsh = sub.merge(sp500_companies, left_on='cik', right_on='cik_num', how='inner').adsh.drop_duplicates()
...
# filter 1: ['n_obs'] > n_min_obs
# filter 2: row_number_date == 1
# filter 3: row_number_year == 1